...
|---|---|---|---|
| **Spawn** | `claude-remote` | Spawn Remote Control sessions in detached tmux panes and get the claude.ai/code URL — works agent-side too: a running Claude can spawn more Claudes without leaving its own session | [claude-remote-launcher](https://github.com/superbereza/claude-remote-launcher) |
...
`claude-remote` (you — or a Claude that needs a helper: agents use it to bud
off fresh sessions in other folders mid-task), **register** the important ones
with `claude-keep add` so they survive reboots, and when work needs to follow
you — **teleport** the session to the laptop and back, history intact.
